...
public class NetworkClientAdapter : MonoBehaviour
{
    ...
    {
        Debug.Log($"Client failed connection to server: {SocketAddress}");
    }
    ...
    {
        var connectionEstablished = Client.Connect(SocketAddress);
        if (connectionEstablished)
        {
            var salutationMessage = Message.Create(MessageSendMode.Reliable);
            var msg = new PlayerSalutationMessage
            {
                PlayerName = playerName
            };
            salutationMessage.Add(msg);
